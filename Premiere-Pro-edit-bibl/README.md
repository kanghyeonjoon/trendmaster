...
**드래그&드롭으로 실행하기 (명령어 없이):**
`여기에_영상을_끌어놓으세요.bat` 파일 아이콘 위에 영상을 끌어다 놓으면 바로 편집됩니다. 여러 개를 한꺼번에 끌어놓으면 순서대로 처리되고, 끝나면 결과(output) 폴더가 자동으로 열려요.

**잘못 잘린 부분 되살리기:**
"여기 왜 잘렸지?" 싶으면 — 잘린 조각은 버려지지 않습니다.
1. `output/..._rejected.xml`을 프리미어로 가져오기(Ctrl+I) → **"[버린컷]" 시퀀스**가 생김 (잘려나간 조각들만 모여 있음)
2. 되살릴 조각을 거기서 복사(Ctrl+C) → 본편 시퀀스의 해당 위치에 붙여넣기
3. 어디가 잘렸는지는 본편 타임라인의 **"재테이크 제거됨" 마커**가 알려줍니다 (마커 코멘트에 잘린 내용 표시)
