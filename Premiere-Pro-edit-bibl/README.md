...
**프리미어로 가져오기 (수동, 3단계):**
1. `파일 > 가져오기`(윈도우 Ctrl+I / 맥 Cmd+I) → `output/..._cut.xml` 선택 → 프로젝트 패널에 생긴 시퀀스 더블클릭 (컷·음량정리 완료된 러프컷)
2. 자막은 `output/..._cut.srt`(또는 `.ass`)을 같은 방법으로 가져와 → 타임라인 자막(캡션) 트랙 위로 드래그 → 프로그램 모니터의 **CC 버튼 → 캡션 표시** 켜기
3. (선택) 타임라인 전체 선택 → `Ctrl+Shift+D`(맥 Cmd+Shift+D) 로 모든 컷에 오디오 전환 적용 (클릭음 제거)

> 참고: `premiere/import_to_premiere.jsx`(자동 가져오기 스크립트)는 **프리미어에서 단독 실행할 수 없습니다** — 프리미어에는 애프터이펙트 같은 `파일 > 스크립트` 메뉴가 없어 CEP/UXP 패널이 있어야 돌아갑니다. 향후 패널을 붙일 때 재사용할 로직으로 보관만 해둔 파일이니, 지금은 위 수동 3단계를 쓰세요.
