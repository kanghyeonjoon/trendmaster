...
**프리미어에서 — ① 원클릭 스크립트 (권장, 한 번에):**
1. 프리미어 `파일 > 스크립트 > 스크립트 파일 실행`(File > Scripts > Run Script File) → `premiere/import_to_premiere.jsx` 선택
2. 끝. 방금 편집한 **시퀀스 + 자막(캡션 트랙)이 자동으로** 들어오고 시퀀스가 열립니다.
   - 자주 쓰면 이 `.jsx`를 프리미어 Scripts 폴더에 복사 → `File > Scripts` 메뉴에 상주(메뉴 클릭 한 번).
   - 자막이 화면에 안 보이면 프로그램 모니터의 **CC 버튼 → 캡션 표시** 켜기.

**프리미어에서 — ② 수동 (스크립트 안 쓸 때):**
